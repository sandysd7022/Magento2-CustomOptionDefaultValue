...
class Option extends \Magento\CatalogImportExport\Model\Import\Product\Option
{
    ...
    const COLUMN_ROW_IS_DEFAULT = '_custom_option_row_is_default';

    const COLUMN_ROW_EXTRA_INFO = '_custom_option_row_extra_info';
    ...
    private function processOptionRow($name, $optionRow)
    {
        ...
        // custom by sandy for save value of is_default field 
        if (isset($optionRow['is_default'])) {
            $result[self::COLUMN_ROW_IS_DEFAULT] = $optionRow['is_default'];
        }

        if (isset($optionRow['extra_info'])) {
            $result[self::COLUMN_ROW_EXTRA_INFO] = $optionRow['extra_info'];
        }
    ...
    private function restoreOriginalOptionTypeIds(array &$typeValues, array &$typePrices, array &$typeTitles)
    {
        foreach ($typeValues as $optionId => &$optionTypes) {
            foreach ($optionTypes as &$optionType) {
                ...
                $optionTypeId = $optionType['option_type_id'] ?? null;
                if (!$optionTypeId) {
                    continue; // or handle as new option
                }
    ...
    protected function _getSpecificTypeData(array $rowData, $optionTypeId, $defaultStore = true)
    {
        ...
        if (!empty($rowData[self::COLUMN_ROW_TITLE]) && $defaultStore && empty($rowData[self::COLUMN_STORE])) {
            $valueData = [
                ...
                'is_default' => empty($rowData[self::COLUMN_ROW_IS_DEFAULT]) ? 0 : 1,
                'extra_info' => !empty($rowData[self::COLUMN_ROW_EXTRA_INFO]) ? $rowData[self::COLUMN_ROW_EXTRA_INFO] : '',
            ];
